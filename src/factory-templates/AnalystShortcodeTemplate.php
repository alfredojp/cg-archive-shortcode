...
class AnalystShortcodeTemplate implements ArchiveShortcodeTemplate {

	public function buildArchiveShortcodePage($archiveShortcode) {

		// Filters data
		$analysts_options = $archiveShortcode->taxomony_options['analyst'] ?? '';
		...
		$custom_filter_title = $archiveShortcode->getData( 'archive_filter_title' ) ?: __( 'Analyst Report library', 'cg-archive-shortcode' );
		$pageContent = file_get_contents( WP_PLUGIN_DIR . '/cg-archive-shortcode/page-templates/analyst-report.php' );
		...
		$page = str_replace(
			[
				...
				'{{ filterLabel }}',
				'{{ filterResultsLabel }}',
				'{{ filterDropdownAnalystLabel }}',
				'{{ filterDropdownAnalystPlaceholder }}',
			...
			[
				wp_kses_post( isset( $custom_filter_title ) ? $custom_filter_title : __( 'Analyst Report library', 'cg-archive-shortcode' ) ),
				...
				esc_attr__( 'Analyst', 'cg-archive-shortcode' ),
				esc_attr__( 'Analyst', 'cg-archive-shortcode' ),
				$analysts_options,
				...
				esc_html__( 'Show 10 more reports', 'cg-archive-shortcode' ),
		...
		if ( ! in_array( 'analyst', $custom_filter_options ) ) { // phpcs:ignore
			$analystFilterHtml = $archiveShortcode->get_string_between($page, '<!-- filterDropdownAnalystStart -->', '<!-- filterDropdownAnalystEnd -->');

			$page = str_replace($analystFilterHtml, '', $page);
		}
